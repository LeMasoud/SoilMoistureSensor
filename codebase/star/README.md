...
# ستاره
توپولوژی ستاره در شبکه‌های مختلف به صور متفاوتی قابل پیاده‌سازی می‌باشد، لیکن همگان در ارتباط یک به همه اشتراک دارند.
در این راهکار حتی ارتباط چند حسگر با یک نقطه دسترسی نیز می‌تواند بنوعی ستاره محسوب شود.
حال اگر طبقه‌بندی محصول محور را در نظر بگیریم، این توپولوژی تنها در صورتی ممکن خواهد شد که گیت‌وی نیز بخشی از محصولات ویرانیک محسوب شود.
انتخاب استاندارد‌های پروتکل و نحوه پیام‌رسانی در این مدل بر عهده گروه تحقیق و توسعه محصولات می‌باشد.

در مجموع هر پروژه مبتنی بر فناوری دیجیتال شامل کد و برنامه‌هایی می‌شود.

هر بخش از کدنویسی محصول شامل پودمان‌های مشخصی می‌شوند که در ادامه بعنوان راهنما به برخی از آنها اشاره می‌شود.
...
if ((Second - 1) % 5 == 0 && pulseTimer) {      //if the second is a multiple of 5 and the pulseTimer is 1
    ...
    transmitUDP();                              //transmit the UDP packet

    pulseTimer = false;                         //set the pulseTimer to 0
}                                               //end of if
```

هماهنگونه که مشاهده می‌کنید، هر پنج ثانیه نسبت به خاموش نمودن چراغ تعبیه شده بر روی ESP اقدام نموده و تابع مرتبط با ارسال داده را فراخوانی می‌نماییم.
